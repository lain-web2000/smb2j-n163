...
SetFreq_N163:
      ldy #$00               ;if branched here, start off by resetting the phase
      ldx #$79
NoteLoop:
      stx N163SND_ADDRESS
      sty N163SND_DATA
      ...
      lda #$00
      sta N163SND_DATA
      lda #$7a               ;now set the frequency regs for N163 channel
      sta N163SND_ADDRESS
      lda N163FreqLookupTbl,y
      sta N163SND_DATA
      lda #$78
      sta N163SND_ADDRESS
      lda N163FreqLookupTbl+1,y
      sta N163SND_DATA
      rts
...
LoadHeader:
        ...
        lda EventMusicBuffer         ;are we playing victory music?
        cmp #VictoryMusic
        bne InitSoundCounters        ;if not, continue to init sound
        lda MusicHeaderData+6,y      ;otherwise load header data for N163 sound
        sta MusicOffset_N163SND
...
InitSoundCounters:
        ...
        lda EventMusicBuffer         ;are we playing victory music?
        cmp #VictoryMusic
        bne InitMusicOffset          ;if not, continue to init music data offset
        lda #$01
        sta N163SND_LenCounter       ;otherwise init music note counter for N163 audio
InitMusicOffset:
...
HandleTriangleMusic:
        ...
        bne HandleN163Music       ;is it time for more data?
...
TriNoteHandler:
          ...
          and #%00001010
          beq HandleN163Music     ;if playing any other primary, or death or d4, go on to N163 routine
...
HandleN163Music:
    lda EventMusicBuffer         ;are we playing victory music?
    cmp #VictoryMusic
    bne DoNoiseMusic             ;if not, don't bother with N163 audio
    lda MusicOffset_N163SND      ;if no offset loaded, skip to handle noise channel
    bne CheckForCutoff
DoNoiseMusic:
    jmp HandleNoiseMusic

CheckForCutoff:
    lda N163SND_LenCounter       ;check to see if length at specific point in note
    cmp #$02                     ;if not, skip this part
    bne RunN163Channel
    lda #$7f                     ;otherwise perform note cutoff
    sta N163SND_ADDRESS
    lda #$00
    sta N163SND_DATA
RunN163Channel:
    dec N163SND_LenCounter       ;if length not expired, skip ahead
    bne N163SND_EnvModRun
    ldy MusicOffset_N163SND
    inc MusicOffset_N163SND      ;get next byte in music data
    lda (MusicData),y
    bpl N163SND_NoteHandler      ;if positive, branch to process note
    jsr ProcessLengthData        ;otherwise process length
    sta N163SND_LenBuffer
    ldy MusicOffset_N163SND
    inc MusicOffset_N163SND      ;get next byte in music data (must not be length byte!)
    lda (MusicData),y
N163SND_NoteHandler:
    jsr SetFreq_N163             ;play a note on the N163 channel
    tay
    bne InitialEnvData           ;if frequency high was nonzero, branch
    lda #$7f                     ;otherwise play a rest, use zero from frequency low data
    sta N163SND_ADDRESS
    lda #$00
    sta N163SND_DATA
InitialEnvData:
    ldy #$00                     ;init envelope counter offset
    sty N163SND_VolumeEnvOffset
    lda #$7f
    sta N163SND_ADDRESS
    lda (N163SND_VolumeEnvData),y ;get volume data for the start of the note
    sta N163SND_DATA
    iny
    lda (N163SND_VolumeEnvData),y ;get timing for volume and sweep envelopes
    sta N163SND_VolumeEnvTimer
    sty N163SND_VolumeEnvOffset  ;set current offset
    lda N163SND_LenBuffer
    sta N163SND_LenCounter       ;dump length of note
N163SND_EnvModRun:
    dec N163SND_VolumeEnvTimer   ;if envelope counter not expired, branch to skip this
    bne HandleNoiseMusic
VolumeEnvCtrl:
    inc N163SND_VolumeEnvOffset
    ldy N163SND_VolumeEnvOffset  ;get next byte of data
    lda (N163SND_VolumeEnvData),y
    ldx #$7f
    stx N163SND_ADDRESS
    and #%00001111
    sta N163SND_DATA              ;write to control the volume of N163 channel
    iny
    lda (N163SND_VolumeEnvData),y ;get another byte of data, set counter
    sta N163SND_VolumeEnvTimer
    sty N163SND_VolumeEnvOffset   ;save offset for later
...
FindHeader:
    ...
    sta N163SND_VolumeEnvData
    lda WaveformHeaderData+1,y
    sta N163SND_VolumeEnvData+1
    lda #$80                ;select N163 sound RAM address
    sta N163SND_ADDRESS
    ldy #$00
GWDLoop:
    lda (WaveformData),y    ;write each byte of data to the waveform RAM
    sta N163SND_DATA        ;from start to end
    iny
    cpy #$20
    bne GWDLoop
    rts
...
VictoryPart1AHdr:      .db $30, <VictoryM_P1AData, >VictoryM_P1AData, $3e, $14, $b0, $24, $01
VictoryPart1BHdr:      .db $30, <VictoryM_P1BData, >VictoryM_P1BData, $50, $21, $61, $31, $02
VictoryPart2AHdr:      .db $30, <VictoryM_P2AData, >VictoryM_P2AData, $43, $1c, $b5, $29, $01
VictoryPart2CHdr:      .db $30, <VictoryM_P2CData, >VictoryM_P2CData, $50, $20, $61, $31, $02
VictoryPart2DHdr:      .db $38, <VictoryM_P2DData, >VictoryM_P2DData, $09, $04, $1e, $06, $01
VictoryPart2BHdr:      .db $38, <VictoryM_P2BData, >VictoryM_P2BData, $3a, $10, $9e, $28, $01
...
WaveformHeaderData:
Wave1Hdr:     .db <WaveformData1, >WaveformData1, <VolEnvData1, >VolEnvData1
Wave2Hdr:     .db <WaveformData2, >WaveformData2, <VolEnvData2, >VolEnvData2

WaveformData1:
    .db $00, $00, $11, $21, $32, $44, $86, $da
    .db $ff, $ff, $ef, $cd, $bb, $9a, $89, $67
    .db $66, $87, $99, $ba, $cb, $ed, $ff, $ff
    .db $df, $8a, $46, $34, $22, $11, $01, $00

WaveformData2:
    .db $40, $bb, $a9, $aa, $9a, $ba, $bb, $8a
    .db $d9, $cf, $db, $9e, $24, $b7, $89, $8a
    .db $87, $8a, $b9, $27, $94, $ee, $cb, $df
    .db $99, $ba, $bb, $9b, $aa, $aa, $b9, $4b

VolEnvData1:
    .db $09, $01, $0a, $01, $0c, $01, $0d, $01
    .db $0f, $2e, $0e, $01, $0d, $02, $0c, $01
    .db $0b, $02, $0a, $01, $09, $02, $08, $01
    .db $07, $02, $06, $02, $05, $01, $04, $02
    .db $03, $01, $02, $02, $01, $01, $00, $ff
VolEnvData2:
    .db $0f, $03, $0e, $03, $0d, $04, $0c, $03
    .db $0b, $03, $0a, $04, $09, $03, $08, $04
    .db $07, $05, $06, $03, $05, $04, $04, $03
    .db $03, $03, $02, $04, $01, $03, $00, $ff

N163FreqLookupTbl:
    .db $01, $44, $01, $58, $01, $99, $02, $22
    .db $21, $a8, $23, $ec, $28, $50, $02, $d9
    .db $2d, $40, $2f, $f4, $32, $cc, $35, $90
    .db $03, $cd, $3c, $68, $40, $00, $43, $d0
    .db $47, $d8, $05, $13, $50, $a4, $05, $b2
    .db $5a, $84, $5f, $a8, $06, $c6, $6b, $a4
    .db $07, $9a, $78, $d4, $7f, $c4, $09, $95
    .db $0a, $26, $00, $00
